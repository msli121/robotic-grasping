...
def poses_save_csv(filepath):
    ...
    # 定义一个空列表，用于存储结果
    # 遍历每一行数据
    lines = [float(i) for line in lines if line.strip() for i in line.strip().split(',')]
    matrices = []
...
def compute_T(images_path, corner_point_long, corner_point_short, corner_point_size):
    ...
    for i in range(0, 50):  # 标定好的图片在images_path路径下，从0.jpg到x.jpg   一次采集的图片最多不超过30张，遍历从0.jpg到30.jpg ，选择能够读取的到的图片
        ...
        image = f"{images_path}\\{i}.png"  # windows下路径
...
if __name__ == '__main__':
    images_path = r"C:\code.py\test1\Color"  # 手眼标定采集的标定版图片所在路径
    file_path = r"C:\code.py\test1\pose1.txt"  # 采集标定板图片时对应的机械臂末端的位姿 从 第一行到最后一行 需要和采集的标定板的图片顺序进行对应
    corner_point_long = 11  # 标定板角点数量  长边
    corner_point_short = 8
    corner_point_size = 0.01  # 标定板方格真实尺寸  m

    print("手眼标定采集的标定版图片所在路径", images_path)
    print("采集标定板图片时对应的机械臂末端的位姿", file_path)
    poses_save_csv(file_path)
    rotation_matrix, translation_vector = compute_T(images_path, corner_point_long, corner_point_short,
                                                    corner_point_size)
    ...
    # 将旋转矩阵转换为四元数
    rotation = pyR.from_matrix(rotation_matrix)
